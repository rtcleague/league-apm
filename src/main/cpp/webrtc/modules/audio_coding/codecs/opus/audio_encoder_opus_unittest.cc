...
AudioEncoderOpusConfig CreateConfigWithParameters(
  ...
  const SdpAudioFormat format("webrtc.opus", 48000, 2, params);
...
TEST(AudioEncoderOpusTest, TestConfigDefaults) {
  const auto config_opt = AudioEncoderOpus::SdpToConfig({"webrtc.opus", 48000, 2});
...
TEST(AudioEncoderOpusTest, TestConfigFromInvalidParams) {
  const webrtc::SdpAudioFormat format("webrtc.opus", 48000, 2);
