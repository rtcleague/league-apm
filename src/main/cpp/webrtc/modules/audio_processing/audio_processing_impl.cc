...
// Add using directive for rtc namespace
using rtc::scoped_refptr;
using rtc::RefCountReleaseStatus;
...
int AudioProcessingImpl::ProcessCaptureStreamLocked() {
  ...
  if (submodules_.echo_controller) {
    RTC_LOG(LS_VERBOSE) << "Entered into echo controller";
  ...
  if (submodules_.echo_control_mobile) {
    RTC_LOG(LS_VERBOSE) << "Entered into echo controller Mobile";
    // Ensure that the stream delay was set before the call to the
    // AECM ProcessCaptureAudio function.
    if (!capture_.was_stream_delay_set) {
        RTC_LOG(LS_ERROR) << "Error Stream Delay Not Set";
        return AudioProcessing::kStreamParameterNotSetError;
    }
    RTC_LOG(LS_VERBOSE) << "AEC Processing...";
    
    if (submodules_.noise_suppressor) {
        RTC_LOG(LS_VERBOSE) << "Entered into noise suppressor";
