...
class NoiseSuppressionImpl : public NoiseSuppression {
 public:
  explicit NoiseSuppressionImpl(Mutex* mutex);
 ...
 private:
  ...
  Level level_ GUARDED_BY(mutex_) = kModerate;
  size_t channels_ GUARDED_BY(mutex_) = 0;
  int sample_rate_hz_ GUARDED_BY(mutex_) = 0;
  std::vector<std::unique_ptr<Suppressor>> suppressors_ GUARDED_BY(mutex_);
