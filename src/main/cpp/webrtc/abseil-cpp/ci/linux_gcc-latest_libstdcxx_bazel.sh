...
# This script that can be invoked to test main_driver.webrtc.abseil-cpp in a hermetic environment
...
for std in ${STD}; do
  for compilation_mode in ${COMPILATION_MODE}; do
    for exceptions_mode in ${EXCEPTIONS_MODE}; do
      echo "--------------------------------------------------------------------"
      time docker run \
        --mount type=bind,source="${ABSEIL_ROOT}",target=/main_driver.webrtc.abseil-cpp-ro,readonly \
        --tmpfs=/main_driver.webrtc.abseil-cpp \
        --workdir=/main_driver.webrtc.abseil-cpp \
        --cap-add=SYS_PTRACE \
        --rm \
